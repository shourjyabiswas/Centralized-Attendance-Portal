...
router.post('/upload', upload.single('file'), async (req, res) => {
  try {
    ...
    const { error: uploadError } = await req.supabase.storage.from(bucket).upload(filePath, file.buffer, {
      contentType: file.mimetype,
    })
    if (uploadError) return res.status(400).json({ error: uploadError.message })

    const { data: { publicUrl } } = req.supabase.storage.from(bucket).getPublicUrl(filePath)
...
router.get('/sections/:id', async (req, res) => {
  try {
    let query = req.supabase.from('content_items').select('*').eq('class_section_id', req.params.id).order('created_at', { ascending: false })
...
router.get('/student', async (req, res) => {
  try {
    ...
    let query = req.supabase.from('content_items').select('*, class_sections ( courses ( name, code ) )').in('class_section_id', ids).order('created_at', { ascending: false })
    if (req.query.type) query = query.eq('type', req.query.type)
    const { data, error } = await query
    if (error) return res.status(400).json({ error: error.message })
    return res.json({ data })
